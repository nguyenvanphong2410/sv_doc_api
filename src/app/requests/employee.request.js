...
export const createEmployeeRequest = Joi.object({
...
export const updateEmployeeRequest = Joi.object({
    name: Joi.string().trim().max(MAX_SIZE_NAME).required().pattern(VALIDATE_NAME_REGEX).label("Họ và tên"),
    email: Joi.string()
        .trim()
        .max(MAX_STRING_SIZE)
...
export const changePasswordEmployeeRequest = Joi.object({
...
export const changeStatusEmployeeRequest = Joi.object({
    status: Joi.string()
        .valid(...Object.values(STATUS_ACTIVE))
        .label("Trạng thái")
        .messages({"any.only": "Trạng thái không hợp lệ."}),
